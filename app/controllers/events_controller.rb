class EventsController < ApplicationController
  ...
  def create
    puts "=" * 50
    puts "voici les params"
    puts params
    puts "=" * 50
    date = Date.new(params[:start_date][:year].to_i, params[:start_date][:month].to_i, params[:start_date][:day].to_i)
    @event = Event.new(user_id: current_user.id, start_date: date, duration: params[:duration].to_i, title: params[:title], description: params[:description], price: params[:price].to_i, location: params[:location])
    if @event.save!
